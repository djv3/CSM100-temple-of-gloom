...
class DijkstraTest {
   ...
   * <p>1-2-3 | | 4 5-6 | | 7-8-9
   *
   * <p>Tile 1 is the start position. Tile 6 is the exit.
   *
   * <p>Tile 2 has 533 gold, tile 3 has 518 gold and tile 4 has 398 gold. Other tiles have no gold.
   *
   * <p>Edges have the following weights: 1-2: 11, 2-3: 9 1-4: 7, 3-5: 6 5-6: 11 4-7: 3, 5-9: 7 7-8:
   * 14, 8-9: 7
   */
  @BeforeEach
  void setUp() {
    ...
    // (using the formula used by the program: timeForShortestPath + 0.3 * (MAX_EDGE_WEIGHT + 1) *
    // cavern.numOpenTiles() / 2)
    timeRemaining = 58;
    timeTakenForShortestPath = 37;

    dijkstra = new Dijkstra(cavern.getGraph(), timeRemaining);
  }

  @Test
  void testBestPath() {
    List<Node> actualPath = dijkstra.bestPath(cavern.getNodeAt(1, 1), cavern.getEntrance());
  ...
  void testTotalGoldOnMap() {
    assertEquals(goldOnMap, dijkstra.totalGoldOnMap());
  }
  ...
  void testTimeTakenToTraversePath() {
    assertEquals(
        timeTakenForShortestPath,
        EscapeAlgorithm.timeTakenToTraversePath(cavern.getNodeAt(1, 1), shortestPath));
  }
  ...
  void testFindPathToClosestNodeWithGold() {
    ...
    List<Node> actualPath =
        dijkstra.findPathToClosestNodeWithGold(
            cavern.getNodeAt(1, 1), cavern.getNodeAt(2, 4), new ArrayList<>());
    List<Node> expectedPath = new ArrayList<>();
    expectedPath.add(cavern.getNodeAt(2, 1));
    assertEquals(expectedPath, actualPath);
    ...
    actualPath =
        dijkstra.findPathToClosestNodeWithGold(
            cavern.getNodeAt(3, 3), cavern.getNodeAt(2, 4), new ArrayList<>());
    ...
    actualPath =
        dijkstra.findPathToClosestNodeWithGold(
            cavern.getNodeAt(3, 3), cavern.getNodeAt(2, 4), visitedNodes);
    expectedPath = new ArrayList<>();
