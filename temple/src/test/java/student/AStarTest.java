...
class AStarTest {
  ...
  void testBestPath() {
    ...
    // This is because we are making the best effort to collect from the nodes with the most gold
    // first.
    // This means that because tile 4 has the lowest gold it is de-prioritised, so by the time we
    // reach
    // tile 3 we no longer have enough time to make it to tile 2 and then to the exit. It's worth
    // noting
    // that we could improve the algorithm by getting all permutations of the top nodes list and
    // then
    // recursively determine the best path for each permutation. This would be very expensive
    // though, as
