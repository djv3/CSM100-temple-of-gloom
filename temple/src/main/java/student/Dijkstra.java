...
/**
 * A maze escape algorithm using Dijkstra's algorithm to calculate the shortest path between 2
 * nodes.
 */
public class Dijkstra extends EscapeAlgorithm {

  /**
   * Constructor that takes the current game state (which should be post-orb-retrieval, pre-escape
   * movement).
   *
  ...
  /**
   * A method that takes Dijkstra's shortest path, checks if there are any tiles with gold adjacent
   * to the path, and modifies the path to collect that gold and return to the original path.
   *
  ...
  /**
   * A method that checks if there's extra time to collect gold after considering Dijkstra's
   * shortest path to the exit, and if so collects the nearest gold and repeats.
   *
   * @param _startNode = the node from which to calculate the path
   * @param _endNode = the node to which to calculate the path (i.e. the exit)
    ...
    while (!needToGetOut) {
      ...
      if (totalGoldOnPath(wanderingPath) < totalGoldOnMap()) {
        ...
        if (wanderingPath.size() == 0) {
          ...
          pathFromStartToGold =
              findPathToClosestNodeWithGold(
                  wanderingPath.get(wanderingPath.size() - 1), _endNode, wanderingPath);
        }

        // Find the path from the nearest gold to the end
        List<Node> pathFromGoldToEnd =
            shortestPath(pathFromStartToGold.get(pathFromStartToGold.size() - 1), _endNode);
        ...
        if (wanderingPath.size() == 0) {
          ...
          timeForCurrentPathToGold =
              timeTakenToTraversePath(
                  wanderingPath.get(wanderingPath.size() - 1), pathFromStartToGold);
        }
        timeForGoldToEnd =
            timeTakenToTraversePath(
                pathFromStartToGold.get(pathFromStartToGold.size() - 1), pathFromGoldToEnd);

        // If there's time to go and get the gold...
        if (timeForStartToEndOfCurrentPath + timeForCurrentPathToGold + timeForGoldToEnd
            < timeRemaining) {
          // ... go and get it
          ...
          if (wanderingPath.size() == 0) {
            ...
            wanderingPath.addAll(
                shortestPath(wanderingPath.get(wanderingPath.size() - 1), _endNode));
          }
   ...
   */
  public List<Node> findPathToClosestNodeWithGold(
      Node _startingNode, Node _exit, List<Node> _visitedNodes) {
    ...
    while (target == null) {
      ...
      if (neighboursWithGold.size() == 0) {
        ...
        for (Node neighbour : neighboursWithGold) {
          int nodeDistanceFromExit =
              timeTakenToTraversePath(neighbour, shortestPath(neighbour, _exit));
