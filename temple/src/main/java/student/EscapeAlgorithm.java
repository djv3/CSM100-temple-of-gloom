...
/**
 * An abstract class representing an algorithm to calculate paths in an escape maze (i.e. with full
 * visibility and gold present on some tiles).
 */
public abstract class EscapeAlgorithm extends Algorithm {
   ...
   * made (as should be the case when using an algorithm to determine the best route to take).
   *
  ...
  /**
   * A method to calculate how much gold is present on a given path (i.e. collection of Node
   * objects).
   *
  ...
  /**
   * A method to calculate how much time it will take to traverse a given path (i.e. collection of
   * contiguous Node objects). This is calculated using the Edge lengths between each node in
   * sequence.
   *
