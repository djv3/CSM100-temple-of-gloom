...
/**
 * Contains three methods to be re-used during implementation of particular search algorithm
 * {@param entrePoint} is the start node for search
 */
...
public abstract class ExploreAlgorithm extends Algorithm {
    ...
    /**
     * Converts data<NodeStatus> Collection of neighbours{@param neighbours}  to list of node records NodeA
     * Uses current node record {@param current} as a parent in record NodeA for all neighbours
    ...
    /**
     * Validates if the concrete node {@param m} is in the given list {@param list}
     * Checks records by nodeID parameter
     */
