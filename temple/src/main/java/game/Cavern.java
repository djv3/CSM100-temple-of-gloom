...
/**
 * An instance represents a Cavern that the explorer can navigate through. The cavern is set up as a
 * grid of Tile objects with a weighted graph of all non-floor tiles. There is an entrance to the
 * cavern and a target location (which may also be the entrance).
 */
public class Cavern {
  ...
  /**
   * Constructor: an instance of size (rws, cls) . Use rand as a source of randomness for the cavern
   * generation. Use {@code edgeWeightGenerator} and {@code goldGenerator} to generate edge weights
   * and gold values. Precondition: {@code targetType} must be either {@code Tile.Type.ORB} or
  ...
  private Cavern(
    ...
    entrance =
        graph.stream().filter((n) -> n.getTile().getType() == Tile.Type.ENTRANCE).findAny().get();
  ...
  /**
   * Constructor: an instance using givenGraph, givenTiles, and target trgt. Preconditions: 1.
   * {@code givenGraph} and {@code givenTiles} represent the same graph (i.e. {@code givenGraph}
   * contains all non-floor nodes in {@code givenTiles} and edges are along the grid). 2. {@code
   * trgt} is a node in {@code givenGraph}.
   */
  ...
  /**
   * Return a new random Cavern of size (rows, cols) with no gold. All edges have weight 1, and an
   * orb is a reasonable distance from the exit. Use rand as a source of randomness for the cavern
   * generation.
   */
  ...
  /**
   * Return a new random Cavern of size (rows, cols) with random gold and edge weights. It is
   * guaranteed that (currentRow, currentCol}) will be an open floor cell. Use rand as a source of
   * randomness for the cavern generation.
   */
  public static Cavern digEscapeCavern(
      int rows, int cols, int currentRow, int currentCol, Random rand) {
  ...
  /**
   * Convert nodeStrList into a Cavern and return it. Precondition: The list of strings is of the
   * format output by {@code serialize()}.
   */
  public static Cavern deserialize(List<String> nodeStrList) {
    String extraInfo = nodeStrList.get(0);
    String[] infoParts = Splitter.on(',').splitToList(extraInfo).toArray(String[]::new);
    String[] dimensions = Splitter.on(':').splitToList(infoParts[0]).toArray(String[]::new);
    int rows = Integer.parseInt(dimensions[0]);
    int cols = Integer.parseInt(dimensions[1]);
    long targetId = Long.parseLong(Iterables.get(Splitter.on(':').split(infoParts[1]), 1));
    ...
    for (String nodeStr : nodeStrList) {
      if (!nodeStr.equals(extraInfo)) {
        ...
        String[] splitInfo = Splitter.on(',').splitToList(nodeInfo).toArray(String[]::new);
    ...
    for (String nodeStr : nodeStrList) {
      ...
      String[] nodeAndEdgeInfo = Splitter.on('=').splitToList(nodeStr).toArray(String[]::new);
      long nodeId = Long.parseLong(Iterables.get(Splitter.on(',').split(nodeAndEdgeInfo[0]), 0));

      Node n = idToNode.get(nodeId);
      tiles[n.getTile().getRow()][n.getTile().getColumn()] = n;
      for (String edgeStr : Splitter.on(',').split(nodeAndEdgeInfo[1])) {
        String[] idAndWeight = Splitter.on('-').splitToList(edgeStr).toArray(String[]::new);
  ...
  /**
   * Add edges to the grid of nodes, tiles, between adjacent non-wall tiles, using
   * edgeWeightGenerator to get edge weights. Precondition: all elements of {@code tiles} are
   * non-null and the grid of nodes has no edges.
   */
  ...
  /**
   * Generate a new random graph that fits within the grid and return the set of nodes. Use
   * goldGenerator to generate gold. If targetType is not ENTRANCE, make the target be some random
   * node that is not the entrance.
   */
  private Set<Node> generateGraph(
      Random rand, Tile.Type targetType, Supplier<Integer> goldGenerator) {
  ...
  /**
   * Randomly determine and return the entrance to the cavern (the only non-wall tile along an edge
   * of the grid).
   */
  private Point getEntrancePoint(Random rand) {
    return switch (rand.nextInt(4)) {
      case 0 -> // North wall
      new Point(rand.nextInt(rows - 2) + 1, 0);
      case 1 -> // South wall
      new Point(rand.nextInt(rows - 2) + 1, cols - 1);
      case 2 -> // West wall
      new Point(0, rand.nextInt(cols - 2) + 1);
      case 3 -> // East wall
      new Point(rows - 1, rand.nextInt(cols - 2) + 1);
  ...
  /**
   * Return the Tile information for (r, c). Precondition: ({@code r}, {@code c}) must be in the
   * grid.
   */
  ...
  /**
   * Return the node at position (r, c). Precondition: ({@code r}, {@code c}) must be in the grid.
   */
   ...
   * between the given node and the target node for this cavern (no path). Precondition: {@code
   * start} must be a node in this cavern.
   */
  ...
  /**
   * Serialize this cavern as a list of strings that can be written to a file. The list can be
   * converted back into a {@code Cavern} using {@code deserialize()}.
   */
