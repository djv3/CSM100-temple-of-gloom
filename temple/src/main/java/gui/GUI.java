...
public class GUI extends JFrame implements Constants {
  ...
  @Getter @Setter private static int framesPerSecond = 60; // Framerate of game (fps)
  ...
  //    private static final int FRAMESPERMOVE = 25;    // How many frames does a single move take
  // us?
  ...
  public GUI(Cavern cavern, int playerRow, int playerCol, long seed) {
    ...
    options =
        new OptionsPanel(
            gameWidth, 0, SCREENWIDTH - gameWidth, (int) (SCREENHEIGHT * INFOSIZE), seed);

    // Create the panel for tile information
    tileSelect =
        new TileSelectPanel(
            gameWidth,
            (int) (SCREENHEIGHT * INFOSIZE),
            SCREENWIDTH - gameWidth,
            (int) (SCREENHEIGHT * (1 - INFOSIZE)),
            this);
    ...
    addComponentListener(
        new ComponentListener() {
          ...
          public void componentResized(ComponentEvent e) {
            ...
            explorer.repaint();
            options.setBounds(
                GAME_WIDTH, 0, SCREENWIDTH - GAME_WIDTH, (int) (SCREENHEIGHT * INFOSIZE));
            tileSelect.updateLoc(
                GAME_WIDTH,
                (int) (SCREENHEIGHT * INFOSIZE),
  ...
  /**
   * Move the player on the GUI to destination dest. Note : This blocks until the player has moved.
  ...
  /**
   * Update the time remaining as displayed on the GUI. timeRemaining is the time remaining before
   * the cave collapses
   */
  ...
  /**
   * Select node n on the GUI. This displays information on that node's panel on the screen to the
   * right.
   */
  ...
  public void displayError(String e) {
    ...
    errorFrame.setLocation(
        new Point(
            getX() + getWidth() / 2 - ERRORWIDTH / 2, getY() + getHeight() / 2 - ERRORHEIGHT / 2));
