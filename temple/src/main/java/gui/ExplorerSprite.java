...
/**
 * Responsible for managing the explorer and drawing it on the screen. Handles functions to update
 * the explorer and update its drawing as well.
 */
public class ExplorerSprite extends JPanel {
  ...
  private final BlockingQueue<MovePair>
      queuedMoves; // List of moves we need to make to get to the goal location
  ...
  /**
   * Return the explorer's row on the grid. Will remain the explorer's old position until the
   * explorer has completely arrived at the new one.
   */
  public int getRow() {
    return row;
  }
  ...
  /**
   * Return the explorer's column on the grid. Will remain the explorer's old position until the
   * explorer has completely arrived at the new one.
   */
  public int getCol() {
    return col;
  ...
  /**
   * Return the the direction the current location (row, col) to (goalRow, goalCol). If already
   * there, return the current direction.
   */
