...
/**
 * An instance is responsible for drawing the underlying maze on the screen. The MazePanel should
 * contain only static images that don't need to change unless the screen is redrawn.
 */
public class MazePanel extends JPanel {
  ...
  public MazePanel(Cavern cav, int screenWidth, int screenHeight, GUI gui) throws IOException {
    ...
    try {
      path =
          ImageIO.read(ClassLoader.getSystemResourceAsStream(PATH_PATH)); // new File(PATH_PATH));
    ...
    for (int row = 0; row < cavern.getRowCount(); row++) {
      for (int col = 0; col < cavern.getColumnCount(); col++) {
        ...
        if (cavern.getTileAt(row, col).getType() != Tile.Type.WALL) {
          ...
          page.drawImage(path, tileWidth * col, tileHeight * row, tileWidth, tileHeight, null);
          // Darken this tile if we haven't been there yet
          if (!visited[row][col]) {
            page.fillRect(tileWidth * col, tileHeight * row, tileWidth, tileHeight);
          }
          // If this is the goal, draw the orb
          if (cavern.getTileAt(row, col).getType() == Tile.Type.ORB) {
            page.drawImage(orb, tileWidth * col, tileHeight * row, tileWidth, tileHeight, null);
          }
          ...
          if (cavern.getTileAt(row, col).getType() == Tile.Type.ENTRANCE) {
            page.drawImage(
                entrance, tileWidth * col, tileHeight * row, tileWidth, tileHeight, null);
          }
        }
        // This is a wall
        else {
          page.drawImage(wall, tileWidth * col, tileHeight * row, tileWidth, tileHeight, null);
        }
      }
    }
  }
